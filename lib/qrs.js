...
module.exports = class QlikSenseRepository {
   ...
   */
  initialize(schema) {
    const filePath = schema || path.join(__dirname, '../schemas/3.2.2.json');
 ...
 */
  ...
  /**
   * @static incrementTimeByMilliseconds - increment time by milliseconds
   *
   * @param {String} time String value in ISO 8601 timestamp format  YYYYMMDDThhmmss.fffK
   * @param {Number} ms   milliseconds
   *
   * @return {String} String value in ISO 8601 timestamp format  YYYYMMDDThhmmss.fffK
   */
  static incrementTimeByMilliseconds(time, ms) {
    const parsedTime = new Date(time);
    parsedTime.setTime(parsedTime.getTime() + ms);
